...
function reverseWords(str) {
  const arr = str.split(' ');
  const reversedElementArr = [];
  arr.forEach(function reverseElement(element) {
    reversedElementArr.push(element.split('').reverse().join(''));
  });
  return reversedElementArr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  while (i < str.length) {
    ...
    if (str[i] === '!' || str[i] === ' ' || str[i] === '?') {
      result += str[i];
    } else if (alphabetFirstPart.indexOf(str[i].toUpperCase()) !== -1) {
      preResult =
        alphabetSecondPart[alphabetFirstPart.indexOf(str[i].toUpperCase())];
    } else {
      preResult =
        alphabetFirstPart[alphabetSecondPart.indexOf(str[i].toUpperCase())];
    }
    if (isUpperCase(str[i])) {
      result += preResult;
    } else {
      result += preResult.toLowerCase();
    }
    i += 1;
  }
